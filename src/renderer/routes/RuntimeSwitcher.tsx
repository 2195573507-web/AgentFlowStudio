    ...
    <div className="mx-auto max-w-7xl space-y-5 p-6">
      <section className="surface-card p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div><h1 className="text-2xl font-bold">Runtime Switcher</h1><p className="mt-2 text-sm text-[var(--text-secondary)]">Generate redacted runtime profiles for Codex, Claude Code, Continue-style CLIs, and custom OpenAI-compatible clients.</p></div>
          <Button variant="secondary" onClick={load} icon={<RefreshCw className="h-4 w-4" />}>Regenerate</Button>
        </div>
      </section>
      ...
      <div className="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
        <div className="space-y-2">
          ...
          {profiles.length === 0 && <EmptyState icon={Terminal} title="No runtime profiles" description="Configure or start the gateway, then regenerate profiles." />}
        </div>
        {selected && (
          <SurfaceCard className="p-5">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><h2 className="text-lg font-semibold">{selected.name}</h2><div className="mt-2 flex flex-wrap gap-2"><Badge>{selected.kind}</Badge><Badge>{selected.redaction}</Badge><Badge>{selected.model}</Badge></div></div><div className="flex gap-2"><Button size="sm" variant="secondary" onClick={() => void copyToClipboard(selected.command)} icon={<Copy className="h-4 w-4" />}>Copy command</Button><Button size="sm" onClick={() => void exportProfile(selected)} icon={<Download className="h-4 w-4" />}>Export</Button></div></div>
            ...
            <div className="mt-4 rounded-tool border border-[var(--border)] bg-[var(--surface-muted)] p-3 text-sm"><div className="font-semibold">Diagnostics</div>{selected.diagnostics.map((item) => <p key={item} className="mt-1 text-xs text-[var(--text-secondary)]">{item}</p>)}</div>
          </SurfaceCard>
        )}
      </div>
    </div>
